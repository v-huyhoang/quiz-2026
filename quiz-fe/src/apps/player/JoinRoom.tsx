...
export default function JoinRoom() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (status === 422) setError("Tên đội đã được dùng. Vui lòng chọn tên khác.");
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      ...
      >
          ...
          {step === 1 ? (
            ...
            <form onSubmit={handleJoin} className="space-y-5">
              ...
              <div className="flex items-center justify-between bg-primary/5 px-4 py-3 rounded-xl border border-primary/10">
                <div>
                  <p className="text-xs font-bold text-primary uppercase tracking-widest mb-0.5">Mã phòng</p>
                  <p className="text-xl font-black text-primary font-mono leading-none">{roomCode}</p>
                </div>
                {!roomCodeFromUrl && (
                  <button
                    type="button"
              ...
              />
                <div className="flex justify-between mt-1">
                  ...
                  <span className={`text-xs font-bold ${teamName.length > 45 ? "text-secondary" : "text-gray-300"}`}>
                    {teamName.length}/50
                  </span>
                </div>
              </div>
        ...
        <p className="text-center text-xs text-gray-400 mt-5">
          Quét QR code từ màn hình host to tham gia tự động
        </p>
      </motion.div>
    </div>
  );
